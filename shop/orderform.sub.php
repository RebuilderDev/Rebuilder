...
<div id="sod_frm" class="sod_frm_pc">
    ...
    <div class="tbl_head03 tbl_wrap od_prd_list">
        ...
        <tr>
            ...
            <th scope="col">소계</th>
            <th scope="col">포인트</th>
            <?php if (!$rb_special_order_only) { ?><th scope="col">배송비</th><?php } ?>
        </tr>
        </thead>
        <tbody>
        <?php
...
$price_calc = "((" . $price_base . " * ct_qty) + COALESCE(ct_date_extra_price, 0) + COALESCE(ct_date_extra_price2, 0) +
        ...
        <tr>
            ...
            <?php if (!$rb_special_order_only) { ?><td class="td_dvr"><?php echo $ct_send_cost; ?></td><?php } ?>
        </tr>
    ...
    <div class="sod_right">
        ...
        <div id="sod_bsk_tot"<?php echo $rb_special_order_only ? ' class="rb-no-delivery"' : ''; ?>>
            <ul>
                ...
                <?php if (!$rb_special_order_only) { ?><li class="sod_bsk_dvr">
                    <span>배송비</span>
...
$(function() {
    ...
    $(".cp_btn").click(function() {
        ...
        $("#cp_frm").remove();
        var it_id = $cp_btn_el.closest("tr").find("input[name^=rb_order_it_id]").val();
...
function forderform_check(f)
{
    ...
    <?php if($default['de_pg_service'] == 'kcp') { ?>
        ...
        if(f.method.value != "무통장") {
            ...
            var order_data = $.param($(f).serializeArray().map(function(field) {
