...
$rb_order_type_map = array();
if (function_exists('rb_shop_order_types_from_orders')) {
    $rb_order_ids = array();
    while ($rb_order_row = sql_fetch_array($result)) $rb_order_ids[] = $rb_order_row['od_id'];
    if ($rb_order_ids) {
        $rb_order_type_map = rb_shop_order_types_from_orders($rb_order_ids);
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" rowspan="3">주문유형</th>
    ...
    {
        $rb_list_order_type = isset($rb_order_type_map[$row['od_id']])
            ? $rb_order_type_map[$row['od_id']]
            : (function_exists('rb_shop_order_type_from_order')
                ? rb_shop_order_type_from_order($row['od_id'])
                : array('id'=>0, 'short_label'=>'일반'));
        $rb_list_order_type_class = isset($rb_list_order_type['id']) && (int) $rb_list_order_type['id'] === 0 ? 'rb-badge-ok' : 'rb-badge-wait';
    ...
    <tr class="orderlist<?php echo ' '.$bg; ?>">
        ...
        <td rowspan="3"><span class="<?php echo $rb_list_order_type_class; ?>"><?php echo get_text($rb_list_order_type['short_label']); ?></span></td>
