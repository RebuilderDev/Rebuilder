...
// depth가 안 들어왔으면: new=new → 1차, 아니면 2차
if ($depth < 1 || $depth > 3) {
    $depth = ($new === 'new') ? 1 : 2;
}
...
<script>
    // 이 팝업에서 생성될 메뉴의 기본 뎁스 (1/2/3)
    var defaultDepth = <?php echo (int)$depth; ?>;
    ...
    function add_menu_list(name, link, code) {
        ...
        // 뎁스에 따른 TD 클래스 (들여쓰기)
        // 1차: td_category
        // 2차: td_category sub_menu_class
        // 3차: td_category sub_menu_class depth3 (원하면 CSS에서 더 들여쓰기)
        var sub_menu_class = " class=\"td_category\"";
        if (defaultDepth >= 2) {
            sub_menu_class = " class=\"td_category sub_menu_class" + (defaultDepth === 3 ? " depth3" : "") + "\"";
        }
        ...
        // ★ 3차 지원용: depth[] 전송
        list += "<input type=\"hidden\" name=\"depth[]\" value=\"" + defaultDepth + "\">";
        ...
        // ★ 뎁스 + 호출한 화면에 따라 '추가' 버튼 분기
        //  - depth 1: 항상 2차추가 버튼(.btn_add_submenu)
        //  - depth 2: RB 메뉴 화면(opener URL에 rb_menu_list.php 포함)에서만 3차추가(.btn_add_submenu3)
        //  - depth 3: 추가 버튼 없음
        if (defaultDepth === 1) {
            list += "<button type=\"button\" class=\"btn_add_submenu btn_03\">추가</button>\n";
        } else if (defaultDepth === 2) {
            try {
                if (opener && opener.location && opener.location.href.indexOf('rb_menu_list.php') !== -1) {
                    list += "<button type=\"button\" class=\"btn_add_submenu3 btn_03\">추가</button>\n";
                }
            } catch (e) {
                // opener 접근 실패 시 무시
            }
