...
$footer_link = empty($_POST['bc_footer_link']) ? 0 : 1;
...
$sql = "INSERT INTO rb_business_console_config SET bc_id=1, bc_enabled='".(empty($_POST['bc_enabled']) ? 0 : 1)."', bc_name='".sql_real_escape_string($name)."', bc_default_route='".sql_real_escape_string($route)."', bc_min_level='".max(1, min(10, (int) $_POST['bc_min_level']))."', bc_show_point='".(empty($_POST['bc_show_point']) ? 0 : 1)."', bc_footer_link='".$footer_link."', bc_partner_policy='".sql_real_escape_string($policy)."', bc_support_url='".sql_real_escape_string($support_url)."', bc_notice='".sql_real_escape_string(trim(isset($_POST['bc_notice']) ? $_POST['bc_notice'] : ''))."', bc_updated_at='".G5_TIME_YMDHIS."' ON DUPLICATE KEY UPDATE bc_enabled=VALUES(bc_enabled), bc_name=VALUES(bc_name), bc_default_route=VALUES(bc_default_route), bc_min_level=VALUES(bc_min_level), bc_show_point=VALUES(bc_show_point), bc_footer_link=VALUES(bc_footer_link), bc_partner_policy=VALUES(bc_partner_policy), bc_support_url=VALUES(bc_support_url), bc_notice=VALUES(bc_notice), bc_updated_at=VALUES(bc_updated_at)";
sql_query($sql);
