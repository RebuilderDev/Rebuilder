...
function rb_alarm_update_memo_badge(count) {
    ...
    if (count > 0) {
        if (!$badge.length) $badge = $('<span>', {'class':'font-H'}).appendTo($button);
...
function rb_alarm_card_html(data) {
    ...
    var id = eventType === 'memo' ? (parseInt(data.memo_id, 10) || 0) : (parseInt(data.notification_id, 10) || 0);
    if (!id) return '';
...
function show_alarm(data) {
    ...
    if(show_alarm_exist){rb_alarm_finish_current(true);}else{rb_alarm_show_next();}
}
